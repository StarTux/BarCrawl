...
public final class StPatricksNeeds implements Needs {
    private final Map<String, Need> map = new HashMap<>();

    public StPatricksNeeds() {
        for (StPatricksNeed need : StPatricksNeed.values()) {
            map.put(need.getKey(), need);
        }
    }
    ...
    public enum StPatricksNeed implements Need {
